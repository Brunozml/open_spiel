...
def eval_against_random_bots(env, trained_agents, random_agents, num_episodes):
  ...
  for player_pos in range(4):
    if player_pos % 2 == 0:
      cur_agents = [trained_agents[player_pos % num_agents], random_agents[(player_pos+1) % num_agents],
                    trained_agents[(player_pos+2) % num_agents], random_agents[(player_pos+3) % num_agents]]
    else:
      cur_agents = [random_agents[player_pos % num_agents], trained_agents[(player_pos+1) % num_agents],
                    random_agents[(player_pos+2) % num_agents], trained_agents[(player_pos+3) % num_agents]]
