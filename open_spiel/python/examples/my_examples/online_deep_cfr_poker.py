...
def eval_against_random_bots(game, bots, num_episodes):
    ...
    for episode in range(num_episodes):
        bots, cfr_player_index = alternate_starting_player(bots)
...
def alternate_starting_player(bots):
...
def plot_rewards(rewards):
            ...
            plt.legend()
            plt.savefig(f'open_spiel/python/examples/saved_examples/{FLAGS.game}_reward_plot_deepcfr-07-05-2.png')

def plot_exploitability(exploitabilities):
    # Plot the exploitability against iterations
     # plot the rewards
    fig = plt.figure()
    plt.loglog(exploitabilities, color = 'blue')
    plt.xlabel('Iterations')
    plt.ylabel('Exploitability')
    plt.title(f'{FLAGS.game}:  Exploitability vs Iterations')
    plt.savefig(f'open_spiel/python/examples/saved_examples/{FLAGS.game}_exp_plot_deepcfr-07-05-2.png')
...
def plot_iteration_times(iteration_times):
    ...
    plt.savefig(f'open_spiel/python/examples/saved_examples/{FLAGS.game}_time_plot_deepcfr-07-05-2.png')

### Override the solve method in deep_cfr.py
def solve(self):
        ...
        for i in range(deep_cfr_solver._num_iterations):
            ...
            if i % FLAGS.eval_every == 0:
                if FLAGS.tabular:
                    ...
                    exp = exploitability.nash_conv(game, average_policy) / game.num_players()
                    # Add the current exploitability and iteration to the lists
                ...
                logging.info("Mean episode rewards: {}".format(r_mean))
                # plot the rewards
