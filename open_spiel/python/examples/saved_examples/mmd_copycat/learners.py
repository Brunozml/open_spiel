...
from open_spiel.python.algorithms.tabular_qlearner import QLearner as _QLearner
...
class QLearner(Learner):
    ...
    def update(self) -> None:
        ...
        while not time_step.last():
            ...
            agent_output = self.solvers[player_id].step(time_step)
    ...
    def test_policy(self) -> TabularPolicy: 
        """Return test policy"""
        # q_values = self.solvers[0]._q_values # first player for now
        # tabular_policy = TabularPolicy(self._game)  # You need to create a game object
        # # Update the policy with your Q-values
        # for state, actions in q_values.items():
        #     # Normalize the Q-values into a probability distribution using a softmax
        #     action_probs = np.exp(list(actions.values())) 
        #     action_probs /= np.sum(action_probs)
            
        #     # Update the policy
        #     for action, prob in zip(actions.keys(), action_probs):
        #         tabular_policy.action_probability_array[state, action] = prob
        # return tabular_policy
        def action_probabilities(state):
            legal_actions = state.legal_actions()
            action, probs = self.solvers[0]._get_action_probs(state, legal_actions, 0)
            return probs
        tabular_policy = policy.tabular_policy_from_callable(self._game, action_probabilities)
