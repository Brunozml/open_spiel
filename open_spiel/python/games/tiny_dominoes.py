...
# Lint as python3
""" Dominoes (4 players) implemented in Python. Tiny version of the game, with 3 tiles per player.
...
_NUM_PLAYERS = 4 
_HAND_SIZE = 2 # CHANGED
_MAX_GAME_LENGTH = _HAND_SIZE * _NUM_PLAYERS # CHANGED

_PIPS = [0.0, 1.0, 2.0, 3.0] # CHANGED
_DECK = list(itertools.combinations_with_replacement(_PIPS, 2))[:_MAX_GAME_LENGTH]
_EDGES = [None] + _PIPS # CHANGED
...
class TinyDominoesGame(pyspiel.Game):
  ...
  def new_initial_state(self):
    ...
    return TinyDominoesState(self)

  def make_py_observer(self, iig_obs_type=None, params=None):
    """Returns an object used for observing game state."""
    return TinyDominoesObserver(
        iig_obs_type or pyspiel.IIGObservationType(perfect_recall=False), params
    )


class TinyDominoesState(pyspiel.State):
  ...
  def _apply_action(self, action):
    ...
    if self.is_chance_node():
      ...
      # if current player's hand is empty, game is over
      if not my_hand:
        self._game_over = True 
        return

      # find next player with legal actions
      for i in range(1,5):
        next_idx = (my_idx + i) % 4
        next_legal_actions = self.get_legal_actions(next_idx)

        if next_legal_actions: # next player has legal actions
  ...
  def update_open_edges(self, action):
    """Update the open edge at both ends of the board after an action is played."""
    if not self.open_edges:
...
class TinyDominoesObserver: # TODO : change
...
pyspiel.register_game(_GAME_TYPE, TinyDominoesGame)